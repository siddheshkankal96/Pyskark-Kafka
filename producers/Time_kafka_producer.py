...
Case_DF = pd.read_csv(r'C:\Users\skankal\Desktop\python\Spark\spark project\archive\Time.csv')
columns = [i.strip() for i in list(Case_DF.columns)]

FILE_PATH = "C:\\Users\\skankal\\Desktop\\python\\Spark\\spark project\\archive\\Time.csv"
...
def get_time_instance(file_path):
    df=pd.read_csv(file_path)
    df=df.iloc[:,:]
    times:List[Time]=[]
    for data in df.values:
        time=Time(dict(zip(columns,data)))
        times.append(time)
        yield time

def time_to_dict(time:Time, ctx):
    """
...
def main(topic):
    ...
    schema_str = schema_registry_client.get_schema(schema_id = 100009).schema_str
    # print(schema_str)

    # print(schema_str)
    string_serializer = StringSerializer('utf_8')
    json_serializer = JSONSerializer(schema_str, schema_registry_client, time_to_dict)
    ...
    counter = 0
    try:
        for time in get_time_instance(file_path=FILE_PATH):
            # print(type(time))
            print(time)

            
            producer.produce(topic=topic,
                                key=string_serializer(str(uuid4()), time_to_dict),
                                value=json_serializer(time, SerializationContext(topic, MessageField.VALUE)),
