...
Case_DF = pd.read_csv(r'C:\Users\skankal\Desktop\python\Spark\spark project\archive\Region.csv')
columns = [i.strip() for i in list(Case_DF.columns)]

FILE_PATH = "C:\\Users\\skankal\\Desktop\\python\\Spark\\spark project\\archive\\Region.csv"
...
def main(topic):
    ...
    schema_str = schema_registry_client.get_schema(schema_id = 100017).schema_str
    # print(schema_str)
    ...
    counter = 0
    try:
        for region in get_region_instance(file_path=FILE_PATH):
            # print(type(region))
            print(region)
