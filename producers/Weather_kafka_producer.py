...
Case_DF = pd.read_csv(r'C:\Users\skankal\Desktop\python\Spark\spark project\archive\Weather.csv')
columns = [i.strip() for i in list(Case_DF.columns)]

FILE_PATH = "C:\\Users\\skankal\\Desktop\\python\\Spark\\spark project\\archive\\Weather.csv"
...
def get_weather_instance(file_path):
    df=pd.read_csv(file_path)
    df=df.iloc[:,:]
    weathers:List[Weather]=[]
    for data in df.values:
        weather=Weather(dict(zip(columns,data)))
        weathers.append(weather)
        yield weather

def weather_to_dict(weather:Weather, ctx):
    """
...
def main(topic):
    ...
    schema_str = schema_registry_client.get_schema(schema_id = 100013).schema_str
    # print(schema_str)

    # print(schema_str)
    string_serializer = StringSerializer('utf_8')
    json_serializer = JSONSerializer(schema_str, schema_registry_client, weather_to_dict)
    ...
    counter = 0
    try:
        for weather in get_weather_instance(file_path=FILE_PATH):
            # print(type(weather))
            print(weather)
            ...
            producer.produce(topic=topic,
                                ...
                                value=json_serializer(weather, SerializationContext(topic, MessageField.VALUE)),
