...
Case_DF = pd.read_csv(r'C:\Users\skankal\Desktop\python\Spark\spark project\archive\TimeAge.csv')
columns = [i.strip() for i in list(Case_DF.columns)]

FILE_PATH = "C:\\Users\\skankal\\Desktop\\python\\Spark\\spark project\\archive\\TimeAge.csv"
...
class TimeAge:   
    ...
    @staticmethod
    def dict_to_timeAge(data:dict,ctx):
        return TimeAge(record=data)

    def __str__(self):
        return f"{self.record}"


def get_timeAge_instance(file_path):
    df=pd.read_csv(file_path)
    df=df.iloc[:,:]
    timeAges:List[TimeAge]=[]
    for data in df.values:
        timeAge=TimeAge(dict(zip(columns,data)))
        timeAges.append(timeAge)
        yield timeAge

def timeAge_to_dict(timeAge:TimeAge, ctx):
    """
...
def main(topic):
    ...
    schema_str = schema_registry_client.get_schema(schema_id = 100010).schema_str
    # print(schema_str)

    # print(schema_str)
    string_serializer = StringSerializer('utf_8')
    json_serializer = JSONSerializer(schema_str, schema_registry_client, timeAge_to_dict)
    ...
    counter = 0
    try:
        for timeAge in get_timeAge_instance(file_path=FILE_PATH):
            # print(type(timeAge))
            print(timeAge)

            
            producer.produce(topic=topic,
                                key=string_serializer(str(uuid4()), timeAge_to_dict),
                                value=json_serializer(timeAge, SerializationContext(topic, MessageField.VALUE)),
