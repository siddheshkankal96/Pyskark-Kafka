...
Case_DF = pd.read_csv(r'C:\Users\skankal\Desktop\python\Spark\spark project\archive\PatientInfo.csv')
columns = [i.strip() for i in list(Case_DF.columns)]

FILE_PATH = "C:\\Users\\skankal\\Desktop\\python\\Spark\\spark project\\archive\\PatientInfo.csv"
...
class PatientInfo:   
    ...
    def dict_to_case(data:dict,ctx):
        return PatientInfo(record=data)

    def __str__(self):
        return f"{self.record}"


def get_patientInfo_instance(file_path):
    df=pd.read_csv(file_path)
    df=df.iloc[:,:]
    patientInfos:List[PatientInfo]=[]
    for data in df.values:
        patientInfo=PatientInfo(dict(zip(columns,data)))
        patientInfos.append(patientInfo)
        yield patientInfo

def patientInfo_to_dict(patientInfo:PatientInfo, ctx):
    """
...
def main(topic):
    ...
    schema_str = schema_registry_client.get_schema(schema_id = 100016).schema_str
    # print(schema_str)

    # print(schema_str)
    string_serializer = StringSerializer('utf_8')
    json_serializer = JSONSerializer(schema_str, schema_registry_client, patientInfo_to_dict)
    ...
    counter = 0
    try:
        for patientInfo in get_patientInfo_instance(file_path=FILE_PATH):
            # print(type(patientInfo))
            print(patientInfo)

            
            producer.produce(topic=topic,
                                key=string_serializer(str(uuid4()), patientInfo_to_dict),
                                value=json_serializer(patientInfo, SerializationContext(topic, MessageField.VALUE)),
